...
#### 背景
`EFK` 插件是`k8s`项目的一个日志解决方案，它包括三个组件：[Elasticsearch](https://github.com/gjmzj/kubeasz/blob/master/docs/guide), [Fluentd](https://github.com/gjmzj/kubeasz/blob/master/docs/guide), [Kibana](https://github.com/gjmzj/kubeasz/blob/master/docs/guide)；Elasticsearch 是日志存储和日志搜索引擎，Fluentd 负责把`k8s`集群的日志发送给 Elasticsearch, Kibana 则是可视化界面查看和检索存储在 Elasticsearch 的数据。
##### 第一部分 Elasticsearch
`es-rbac.yaml`
```
...
```
kubectl create -f es-rbac.yaml  es-service.yaml  es-statefulset.yaml
kubectl create -f fluentd-daemonset.yaml  fluentd-es-configmap.yaml  fluentd-es-rbac.yaml
kubectl create -f kibana-deployment.yaml  kibana-service.yaml
```
访问kibana
通过apiserver访问: `https://xx.xx.xx.xx:6443/api/v1/namespaces/kube-system/services/kibana-logging/proxy/login?`


>>Q&A 
> 1. fluentd daemonset 启动后无容器组服务创建
> 需要给node打上标签
`kubectl label nodes {node_name} beta.kubernetes.io/fluentd-ds-ready="true" `
之后会在打过标签的node上启动fluentd容器
>2. elasticsearch 在实验环境出现OOM 被kubernetes调度器KIlled
这里可以在elasticsearch-statefulset里指定环境变量env里
`- name: ES_JAVA_OPTS`
`value: -Xms256m -Xmx256m`
